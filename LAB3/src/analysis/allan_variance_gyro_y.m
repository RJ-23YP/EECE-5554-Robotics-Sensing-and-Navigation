...
% Assigning angular velocity from the csv file. 
gyroy = field.IMU.gyro_y;
string = 'Angular Velocity - Y';

% Time step
t = 1 / Fs;

% Compute the cumulative sum
theta = cumsum(gyroy, 1) * t;
...
figure
loglog(tau, adev)
title('Allan Deviation Plot for Y');
...
title('Bias Instability for Y');

tauParams = [tauN, tauB];
params = [N, scfB * B];
figure
loglog(tau, adev, tau, [lineN, lineB], '--', ...
tauParams, params, 'o');
title('Noise Parameters for Y');
ylabel('Angular Velocity in Y (deg / s)')
xlabel('Time (s)')